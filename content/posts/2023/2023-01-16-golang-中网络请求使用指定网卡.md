...
从最后两条路由记录可以看到，对于 `192.168.3.0/24` 这个网段的流量会匹配的两个网卡，但由于 第一条路由配置的 `Metric` 的优先级比较高，因此最终流量只会走网卡 `ens33`。

我们在另一台机器 `192.168.3.58` 用 `python3` 搭建一个`HTTPServer`，用命令 `python3 -m http.server 8080` 快速启动一个webserver，这时在原来机器终端使用用 `curl` 发送一个请求。
