...
> 这里的变量 `iter` **不是“存放元素的容器”**，而是一个**记录“当前迭代到哪里”的迭代器对象**。最后打印 iter 的值是指**迭代器**当前还剩下的元素。对于 len() 方法也是类似的工作原理，表示这个 iterator 当前还能够产生多少个元素，总结来说它是工作在迭代器上，而不是变量a。 我们可以重复调用 `a.iter()` 返回多个迭代器，然后再消费这个迭代器。
