...
动态扩缩容主要包括两个层级的动态扩缩容。一个层级是应用本身级别的扩缩容，如HPA或VPA，当应用负载过高时，可以通过HPA多部署几个Pods副本，或者对当前Pod资源配置进行扩容，以此来减少应用负载。

另一层是对集群自身的扩容，如 worker 节点的扩容。如当前部署Pods应用时，出现无可用节点资源可用时，则新加入一些新的节点，并在新节点时对其重建Pods。
